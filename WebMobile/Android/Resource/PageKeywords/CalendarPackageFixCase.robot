...
Resource        CalendarPackageNonStop.robot
Resource        CalendarPackageUpSpeed.robot
...
Verified Button Package Entertainment VIU
    ...
    Web Capture Screen At Verify Point    Verified Button Package Entertainment
# } End

# ตรวจสอบแพ็กเกจจาก Banner
# Start {
Select Register Package Banner Mao Mao Voice
    Click Web Element    ${imgBannerMaoMaoVoice}

Verify Package Banner Mao Mao Voice
    [Arguments]    ${Minute}    ${Baht}    ${Day}
    Run Keyword If    '${ar_LANG}' == 'EN'    Click Web Element    ${btnLanguageEN}
    Web Element Text Should Be    ${lblMaoMaoVoice}    ${txtMaoMaoVoice}
    Web Element Text Should Be    ${lblMinuteMaoMaoVoice}    ${txtMinuteMaoMaoVoice}
    Web Element Text Should Be    ${lblPackPrice}     ${txtPrice${Baht}}
    Web Element Text Should Be    ${lblPackDay}    ${txt${Day}}
    Web Capture Screen At Verify Point    Verify Package Banner Mao Mao Voice  
# } End